...
**据え置き（コンセントに挿しっぱなし）を前提にしています。**
具体的な製品リンク・価格・落とし穴は **[docs/parts.md](docs/parts.md)** にまとめてあります。

まずは「ステップA」の5点だけ揃えれば、体は動かないものの会話は成立します。

| 部品 | 備考 |
|---|---|
| Raspberry Pi 5（4GB で足りる） | 技適マーク付きを。Pi 4 では音声認識モデルを `base` に落とすこと |
| **USB-C 電源 27W (5V/5A)** | 5V/3A だと USB ポートが 600mA に制限され、マイクとスピーカーが不安定になります |
| USB マイク | ドライバ不要のもの。**ReSpeaker 2-Mics HAT は Pi 5 非対応なので避ける** |
| スピーカー | 小型アンプ内蔵のもの |
| microSD 64GB | OS + 音声認識モデル + VOICEVOX で 10GB 前後使います |
| SG90 サーボ ×2 | しっぽ・首 |
